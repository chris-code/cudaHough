...
__global__ void generateGaussGPU(imgT *result, long width, long height, imgT sigma2, imgT normalizationTerm) {
	long x = blockIdx.x * blockDim.x + threadIdx.x;
	long y = blockIdx.y * blockDim.y + threadIdx.y;
...
//	1. But we lose 5% of it, so it doesn't exactly sum to one. Modify the normalization term to compensate for this.
	...
	generateGaussGPU<imgT> <<<blocks, threads>>>(gauss, filterWidth, filterHeight, imgT(pow(sigma, 2.0)),
...
//	Blurr the image with it
	...
	convolveGPU<imgT> <<<blocks, threads>>>(result, image, width, height, gauss, filterWidth, filterHeight,
...
T * computeGradientStrength(T *grayValueImage, long width, long height) {
	...
	assertCheck(cudaGetLastError());
	convolveGPU<T> <<<blocks, threads>>>(gradientY, grayValueImage, width, height, sobelY, 3, 3, 1, 1);
	assertCheck(cudaGetLastError());
